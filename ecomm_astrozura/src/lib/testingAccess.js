...
export function installTestingAccessHeader() {
  ...
  const getPassword = () => getTestingAccessPassword();

  if (!window.__astrozuraTestingFetchPatched && typeof window.fetch === "function") {
  ...
  if (!window.__astrozuraTestingAxiosPatched) {
    axios.interceptors.request.use((config) => {
      const password = getPassword();

      if (password) {
        config.headers = config.headers || {};
        config.headers[TEST_ACCESS_HEADER] = password;
      }

      return config;
    });

    window.__astrozuraTestingAxiosPatched = true;
  }
}
