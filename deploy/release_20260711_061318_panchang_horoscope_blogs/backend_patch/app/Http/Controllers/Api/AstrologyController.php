...
class AstrologyController extends Controller
{
    ...
    public function getDailyHoroscope(Request $request, $sign = null)
    {
        try {
            ...
            $predictionData = is_array($prediction) ? $prediction : [];
            $scoreFromPrediction = function (string $field, string $fallbackSeed) use ($predictionData, $sign, $day, $payload): int {
                if (isset($predictionData[$field]) && is_numeric($predictionData[$field])) {
                    $rating = (float) $predictionData[$field];
                    return (int) max(10, min(100, $rating <= 5 ? round($rating * 20) : round($rating)));
                }

                $seed = $sign . '|' . $day . '|' . ($payload['prediction_date'] ?? now('Asia/Kolkata')->toDateString()) . '|' . $fallbackSeed;
                return 58 + (abs(crc32($seed)) % 33);
            };

            $responsePayload = [
                'daily_prediction' => [
                    'personal' => $predictionData['personal_life'] ?? (is_string($prediction) ? $prediction : 'No personal insights available.'),
                    'profession' => $predictionData['profession'] ?? 'No career insights available.',
                    'health' => $predictionData['health'] ?? 'No health insights available.',
                    'emotions' => $predictionData['emotions'] ?? $predictionData['luck'] ?? 'No emotional insights available.',
                    'travel' => $predictionData['travel'] ?? null,
                    'luck' => $predictionData['luck'] ?? null,
                ],
                'scores' => [
                    'love' => $scoreFromPrediction('emotions_rating', 'love'),
                    'career' => $scoreFromPrediction('profession_rating', 'career'),
                    'health' => $scoreFromPrediction('health_rating', 'health'),
                ],
    ...
    public function getMonthlyHoroscope(Request $request, $sign = null)
    {
        try {
            ...
            $scoreFromPrediction = function (string $field, string $fallbackSeed) use ($prediction, $sign, $monthKey): int {
                if (is_array($prediction) && isset($prediction[$field]) && is_numeric($prediction[$field])) {
                    $rating = (float) $prediction[$field];
                    return (int) max(10, min(100, $rating <= 5 ? round($rating * 20) : round($rating)));
                }

                return 58 + (abs(crc32($sign . '|' . $monthKey . '|' . $fallbackSeed)) % 33);
            };
    ...
    public function yearlyHoroscopeAccess(Request $request)
    {
        $year = (int) ($request->query('year') ?: now('Asia/Kolkata')->year);
        $access = YearlyHoroscopeAccess::query()
            ->where('user_id', $request->user()->id)
            ->where('year', $year)
            ->where('payment_status', 'paid')
            ->where('access_expires_at', '>', now('Asia/Kolkata'))
            ->latest()
            ->first();
    ...
    public function createYearlyHoroscopeAccess(Request $request)
    {
        ...
        $activeAccess = YearlyHoroscopeAccess::query()
            ->where('user_id', $request->user()->id)
            ->where('year', $year)
            ->where('payment_status', 'paid')
            ->where('access_expires_at', '>', now('Asia/Kolkata'))
        ...
        if ($activeAccess) {
            return response()->json([
                'status' => 'success',
                'message' => 'Yearly horoscope access is already active.',
                'data' => $activeAccess,
            ]);
        }

        $access = YearlyHoroscopeAccess::create([
    ...
    public function getPaidYearlyHoroscope(Request $request)
    {
        $validated = $request->validate([
            'sign' => 'required|string|max:32',
            'year' => 'nullable|integer|min:1900|max:2100',
            'la' => 'nullable|string',
        ]);

        $sign = strtolower((string) $validated['sign']);
        $year = (int) ($validated['year'] ?? now('Asia/Kolkata')->year);
        $access = YearlyHoroscopeAccess::query()
            ->where('user_id', $request->user()->id)
            ->where('year', $year)
        ...
        if (!$access) {
            return response()->json([
                'status' => 'error',
                'message' => 'Please unlock yearly horoscope access to view this report.',
        ...
        try {
            $language = $this->resolveRequestedLanguage($request, ['en'], 'en');
            $timezone = (float) config('astrologyapi.default_timezone', 5.5);
            $response = $this->astrologyApi->json("horoscope_prediction/yearly/{$sign}", [
                'timezone' => $timezone,
                'year' => $year,
            ], $language);
            ...
            if (!$response->successful()) {
                return response()->json([
                    'status' => 'error',
                    'message' => $this->extractAstrologyApiError($response, 'Yearly zodiac horoscope unavailable.'),
                ], 200);
            }

            $payload = $response->json();
            $horoscopePayload = $this->normalizeZodiacHoroscopePayload(
    ...
    public function downloadFreeKundliPdf(Request $request)
    {
        ...
        try {
            ...
            if (!$response->successful()) {
                return $this->downloadGeneratedFreeKundliPdf($request, $birthPayload, $language);
            }
            ...
            return $this->downloadGeneratedFreeKundliPdf($request, $birthPayload ?? null, $language ?? 'en');
        }
    }

    private function downloadGeneratedFreeKundliPdf(Request $request, ?array $birthPayload, string $language = 'en')
    {
        if (!$birthPayload) {
            $birthPayload = $this->buildBirthPayload(
                $request->input('date_of_birth') . 'T' . $request->input('time_of_birth') . ':00+05:30',
        ...
        $provider = [
            'birth_details' => $this->safeAstrologyRequest('birth_details', $birthPayload, $language),
            'astro_details' => $this->safeAstrologyRequest('astro_details', $birthPayload, $language),
            'planets' => $this->safeAstrologyRequest('planets', $birthPayload, $language),
        ];
        ...
        $pdf = Pdf::loadView('pdf.free-kundli', [
            ...
            'timeOfBirth' => (string) $request->input('time_of_birth'),
            'placeOfBirth' => (string) $request->input('place_of_birth'),
            'coordinates' => (string) $request->input('coordinates'),
            'provider' => $provider,
        ])->setPaper('a4');

        return $pdf->download($filename . '-kundli.pdf');
    }
        ...
        try {
            ...
            $sections = match ($calculator) {
                ...
                'daily-nakshatra-predictions' => [[
                    ...
                    'summary' => 'Previous, current and next daily Nakshatra prediction modules.',
                    'items' => [
    ...
    public function getLalKitab(Request $request)
    {
        ...
        try {
            ...
            $sections = [[
                'id' => 'lalkitab',
                'title' => 'Lal Kitab',
                'summary' => 'Debt, house and planet-state details generated from the user birth details.',
                'items' => [
    ...
    private function normalizeZodiacHoroscopePayload(array $payload, string $sign, string $period, string $displayDate, string $dateKey): array
    {
        $prediction = $payload['prediction'] ?? $payload['horoscope'] ?? $payload['data'] ?? null;
        $predictionData = is_array($prediction) ? $prediction : [];
        $predictionText = is_string($prediction) ? $prediction : null;

        $scoreFromPrediction = function (string $field, string $fallbackSeed) use ($predictionData, $sign, $period, $dateKey, $payload): int {
            if (isset($predictionData[$field]) && is_numeric($predictionData[$field])) {
                $rating = (float) $predictionData[$field];
                return (int) max(10, min(100, $rating <= 5 ? round($rating * 20) : round($rating)));
            }

            $seed = $sign . '|' . $period . '|' . ($payload['prediction_date'] ?? $dateKey) . '|' . $fallbackSeed;
            return 58 + (abs(crc32($seed)) % 33);
        };

        return [
            'daily_prediction' => [
                'personal' => $predictionData['personal_life'] ?? $predictionData['personal'] ?? $predictionText ?? 'No insights available.',
                'profession' => $predictionData['profession'] ?? $predictionData['career'] ?? 'No career insights available.',
                'health' => $predictionData['health'] ?? 'No health insights available.',
                'emotions' => $predictionData['emotions'] ?? $predictionData['luck'] ?? 'No emotional insights available.',
                'travel' => $predictionData['travel'] ?? null,
                'luck' => $predictionData['luck'] ?? null,
            ...
            'status_label' => 'Favorable',
            'display_date' => $payload['prediction_date'] ?? $displayDate,
            'sign' => $sign,
            'day' => $period,
            'language' => 'en',
            'provider_payload' => $payload,
